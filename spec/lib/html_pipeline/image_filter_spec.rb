...
RSpec.describe HtmlPipeline::ImageFilter do
  ...
  context "src: 1|s" do
    let(:doc) { '<img src="1|s">' }
    it { expect(subject).to eq %(<img src="#{store_dir_prefix}/image/file/1/thumb_test.jpg">) }
  end

  context "src: 1||my_class" do
    let(:doc) { '<img src="1||my_class">' }
    it { expect(subject).to eq %(<img src="#{store_dir_prefix}/image/file/1/large_test.jpg" class="my_class">) }
  end

  context "src: 1|m|my_class" do
    let(:doc) { '<img src="1|m|my_class">' }
    it { expect(subject).to eq %(<img src="#{store_dir_prefix}/image/file/1/medium_test.jpg" class="my_class">) }
  end
